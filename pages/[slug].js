...
function Post(props) {
  ...
  const title = `${props.title} // Gregoire Geoffroy`
  const description = props.description || ''
  const url = `https://gregoiregeoffroy.com/${props.slug}`
  const date = new Date(props.date).toISOString()
  const image = props.image
    ? `https://gregoiregeoffroy.com${props.image}`
    : 'https://gregoiregeoffroy.com/static/images/home-opt.jpg'
...
export async function getStaticProps({ params }) {
  try {
    ...
    const base = isProd ? 'https://gregoiregeoffroy.com' : 'http://localhost:3000'
